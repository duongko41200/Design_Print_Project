<template>
							...
							<div class="z-10 mt-8 flex">
								...
								>
									<div class="p-4 pt-0 bg-zinc-700 h-[400px]">
										<div
											class="flex w-full h-full self-stretch flex-col md:flex-row pb-16 md:pb-0 md:pt-0 flex-1 justify-between"
										>
											<!-- // show detail design -->
											<div
											...
											<!-- // form save design-->
											<div
												class="w-full flex-shrink-0 overflow-hidden text-base flex flex-col h-auto border"
												style="height: fit-content; width: 370px"
											>
												<TransitionChild
													as="template"
											...
											<div class="flex justify-center w-full">
												<div>
													<div class="relative upper-body">
														<img
															:src="
																imageUpperFront
																	? imageUpperFront
																	: require('@/assets/ao_T-shirt.png')
															"
															:class="{ 'h-[160px]': imageUpperFront }"
														/>

														<div class="absolute wrapper-collection">
															<div
																class="absolute icon-upper z-10"
																@click="
																	onShowSelectDesignbyCollection(
																		'upper-body'
																	)
																"
															>
																<icon
																	icon="fa-solid fa-circle-plus text-transparent"
																	size="2xl"
																/>
															</div>
														</div>
													</div>
													<div class="relative lower-body">
														<img
															:src="
																imageLowerFront
																	? imageLowerFront
																	: require('@/assets/pant short front.png')
															"
															:class="{ 'h-[160px]': imageLowerFront }"
														/>
														<div class="absolute wrapper-collection">
															<div
																class="absolute icon-lower z-10"
																@click="
																	onShowSelectDesignbyCollection(
																		'lower-body'
																	)
	...
	<ModalSelectDesign
		:showModal="isShowSelectDesignByCollections"
		:typeCollection="typeCollections"
		@oncloseModal="oncloseModalSelectDesign"
		@onDesignProductDesignUpper="onDesignProductDesignUpper"
		@onDesignProductDesignLower="onDesignProductDesignLower"
	></ModalSelectDesign>
</template>
...
import ModalSelectDesign from './ModalSelectDesign/ModalSelectDesign.vue';
...
export default {
	...
	data() {
		return {
			...
			description: '',
			selected: '',
			isShowSelectDesignByCollections: false,
	...
	methods: {
		...designMappper.mapActions(['getAllCollectionByUser']),
		oncloseModal() {
			...
			this.imageUpperFront = '';
			this.imageUpperBack = '';
			this.imageLowerFront = '';
			this.imageLowerBack = '';
			this.nameDesign = '';
			this.description = ''
		},
		onShowSelectDesignbyCollection(value) {
			console.log(value);
			this.isShowSelectDesignByCollections = true;
			this.typeCollections = value;
		},
		oncloseModalSelectDesign() {
			this.isShowSelectDesignByCollections = false;
		},
		async clickSaveDesign() {
			const params = {
				name: this.nameDesign,
				description: this.description,
				user: this.userInfo.id,
				isPublic: this.selected.type,
				upperBody: this.idUpperBody,
				lowerBody: this.idLowerBody,
			};
			if (
				this.nameDesign == '' ||
				this.idUpperBody == '' ||
				...
				return this.$toast.error('create collection fail', {
					position: 'top-right',
					duration: 2000,
				});
			}
			...
			this.$toast.success('create collection success', {
				position: 'top-right',
				duration: 2000,
			});
			this.oncloseModal();
		...
		onDesignProductDesignUpper(design) {
			console.log('design product design:', design);
			this.imageUpperFront = design.thumbnailFront;
			this.imageUpperBack = design.thumbnailBack;
			this.idUpperBody = design.id;
			this.oncloseModalSelectDesign();
		},
		onDesignProductDesignLower(design) {
			// console.log("design product design:", design)
			this.imageLowerFront = design.thumbnailFront;
			this.imageLowerBack = design.thumbnailBack;
			this.idLowerBody = design.id;
			this.oncloseModalSelectDesign();
		},
	},
	// watch: {
...
<style src="./ModalCollection.css"></style>
